...
static func get_isolating_run_sequences(data):
	...
	for sequence in isolating_run_sequences:
		...
		var _last_level = data['chars'][last_index].level
		var _following_level = data['level'] if last_index == 0 else data['chars'][last_index - 1].level
