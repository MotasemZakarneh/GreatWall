...
func load_theme(theme: ConfigFile):
	...
	#my change start
	$ar_NextIndicatorContainer.rect_position = Vector2.ZERO
	var ar_indicator = get_node("ar_NextIndicatorContainer/NextIndicator")
	ar_indicator.texture = DialogicUtil.path_fixer_load(theme.get_value('next_indicator', 'image', 'res://addons/dialogic/Example Assets/next-indicator/next-indicator.png'))
#	ar_indicator.margin_top = 0 
#	ar_indicator.margin_bottom = 0 
#	ar_indicator.margin_left = 0 
#	ar_indicator.margin_right = 0 
	ar_indicator.rect_scale = Vector2(indicator_scale, indicator_scale)
	#next_indicator.rect_position = theme.get_value('box', 'size', Vector2(910, 167)) - (next_indicator.texture.get_size() * indicator_scale)
#	next_indicator.rect_position -= offset
	ar_indicator.self_modulate = Color('#ffffff')
	ar_indicator.get_node("AnimationPlayer").play(animation)
	#my change end
