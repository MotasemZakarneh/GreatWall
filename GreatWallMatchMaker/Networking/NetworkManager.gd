...
func start_client(_address,_port):
	...
	_er = network.connect("connection_succeeded",self,"_connection_success")
	_er = network.connect("connection_failed",self,"_connection_failed")
	_er = network.connect("server_disconnected",self,"_on_server_disconnected")
	_er = network.connect("peer_connected",self,"_on_peer_connected")
	_er = network.connect("peer_disconnected",self,"_on_peer_disconnected")
...
func _connection_success():
	console.write(name + " :: Connected To " + name +" :: as " +str(network.get_unique_id()) )
...
func _connection_failed():
	console.write(name + " :: Connection Failed To " + name)
...
func _on_server_disconnected():
	console.write(name + " :: server has disconnected")
	pass

remote func r_recieve_player_address(player,_address,_port):
...
func start_server(_port):
	...
	_er = network.connect("peer_connected",self,"_s_on_peer_connected")
	_er = network.connect("peer_disconnected",self,"_s_on_peer_disconnected")
	
	console.write(name + " :: Started As Server To Player :: Port :: " + str(_port))
...
remote func s_request_player_address(player):
